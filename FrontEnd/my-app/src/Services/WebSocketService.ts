...
class WebSocketService {
    ...
    this.client = new Client({
      ...
      onStompError: (frame) => {
        ...
        if (this.isPermissionDeniedMessage(serverMessage)) {
          toast.error(
            "You do not have permission to send messages in this course chat.",
          );
          return;
        }
  ...
  private extractServerErrorMessage(frame: {
  ...
  private isPermissionDeniedMessage(message: string): boolean {
    return /access denied|missing required permission|permission|forbidden|denied/i.test(
      message,
    );
  }
